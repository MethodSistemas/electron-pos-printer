...
async function renderDataToHTML(event, arg) {
  switch (arg.line.type) {
    ...
    case 'free-text':
      try {
        body.appendChild(generatePageFreeText(arg.line));
        // sending msg
        event.sender.send('render-line-reply', { status: true, error: null });
      } catch (e) {
        event.sender.send('render-line-reply', {
    ...
    case 'qrCode':
      try {
        ...
        applyElementStyles(qrCode, {
          textAlign: arg.line.position
            ? '-webkit-' + arg.line.position
            : '-webkit-left',
        });
    ...
    case 'barCode':
      try {
        ...
        const barcodeEl = document.createElementNS(
